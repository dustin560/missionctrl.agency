/**
 * Edge Function: Basic Auth for /proposals/solmax/*
 *
 * Password is read from the Netlify environment variable:
 *   PROPOSAL_PASSWORD_SOLMAX
 *
...
export default async (request, context) => {
  ...
  let password;
  try {
    password = Deno.env.get("PROPOSAL_PASSWORD_SOLMAX");
  } catch (e) {
    console.error("[proposal-auth] Deno.env.get failed:", e && e.message);
    return new Response(
  ...
  if (!password) {
    console.error("[proposal-auth] PROPOSAL_PASSWORD_SOLMAX env var is not set.");
    return new Response(
  ...
  if (authHeader.startsWith("Basic ")) {
    ...
    try {
      ...
      console.error("[proposal-auth] atob decode failed:", e && e.message);
    }
  ...
  return new Response("Authentication required.", {
    status: 401,
    headers: {
      "WWW-Authenticate": 'Basic realm="Solmax Proposal", charset="UTF-8"',
...
export const config = {
  // Covers /proposals/solmax (no slash), /proposals/solmax/ and
  // everything beneath it (assets, sub-paths, etc.)
  path: ["/proposals/solmax", "/proposals/solmax/*"],
};
